...
import kotlinx.android.synthetic.main.item_my_swipe_post.view.*

/**
 * Created by chiranjib on 6/12/17.
 */
class MyPostNewAdapter(var from:String, context: Context?, arrayList: ArrayList<MyPostResponce.UserDataBean>?, myClick: MyOnClick) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup?, viewType: Int): RecyclerView.ViewHolder {
        val v = LayoutInflater.from(parent?.context).inflate(R.layout.item_my_swipe_post, parent, false)
    ...
    class ViewHolder(itemView: View, var from: String) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(position: Int, postData: MyPostResponce.UserDataBean?, myClick: MyOnClick) = with(itemView) {
            ...
            if(from.equals("MyPostNewFragment")){
                itemView.delete_post.visibility = View.VISIBLE
            }else if(from.equals("NewPostFragment")){
                itemView.delete_post.visibility = View.GONE
            }
